...
public class UserConnector {
    ...
    // --- DELETE SETTING ---
    public boolean deleteUserSetting(long authId, String option) {
        String sql = """
                    DELETE FROM settings
                    WHERE auth_id = ? AND option = ?
                """;
        return jdbc.update(sql, authId, option) == 1;
    }

    // --- DELETE ALL SETTINGS FOR USER ---
    public int deleteAllUserSettings(long authId) {
        String sql = """
                    DELETE FROM settings
                    WHERE auth_id = ?
                """;
        return jdbc.update(sql, authId);
    }

    // --- CHECK IF SETTING EXISTS ---
    public boolean settingExists(long authId, String option) {
        String sql = """
                    SELECT EXISTS(
                ...
                """;
        Boolean result = jdbc.queryForObject(sql, Boolean.class, authId, option);
